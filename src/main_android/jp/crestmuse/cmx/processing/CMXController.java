...
//  }
	...
	public void smfread(int iMusic, int iMidi, String filename) {
		try {
			...
			throw new CMXIOException("Cannot read file: " + filename);
		} catch (jp.kshoji.javax.sound.midi.MidiUnavailableException e) {
			throw new DeviceNotAvailableException("MIDI device not available");
		} catch (jp.kshoji.javax.sound.midi.InvalidMidiDataException e) {
...
// for debug 20190624 fujii
    ...
    */
	 ...
	 */
	public void showMidiInChooser(Context context, int resource) {
		showMidiInChooser(0, context, resource);
	}
	 ...
	 * 表示するダイアログボックスの親ウィンドウが不明な場合，<tt>parent</tt>には<tt>null</tt>を指定することもできます．
	 */
	public void showMidiInChooser(final int i, Context context, int resource) {
	    try {
			final List<MidiDevice.Info> midiInDeviceInfo = SoundUtils.getMidiInDeviceInfo();
			AlertDialog.Builder builder = new AlertDialog.Builder(context);
			builder.setTitle("Select MIDI IN Device...")
					.setSingleChoiceItems(
							new ArrayAdapter(context, resource, midiInDeviceInfo),
							0,
