...
public class CMXController implements TickTimer, MIDIConsts {
	 ...
	 * @param seq 読み込むシーケンス
	 */
	public void smfread(int iMusic, int iMidi, Sequence seq) {
		try {
			if (midiouts[iMidi] == null) {
				musicPlayer[iMusic] = new SMFPlayer();
			} else {
	...
	/**
	 * SCCXMLWrapperを指定してSCCXMLドキュメントを標準MIDIファイルに変換して読み込みます．
	 * 
	 * @param scc 読み込むSCCXMLドキュメント
	 * @see #smfread(int, int, SCCXMLWrapper)
	 */
	...
	/**
	 * MIDI出力デバイスの要素番号とSCCXMLWrapperを指定してSCCXMLドキュメントを標準MIDIファイルに変換して読み込みます．
	 * 
	 * @param iMidi MIDI出力デバイスの要素番号(0-255)
	 * @param scc 読み込むSCCXMLドキュメント
	 * @see #smfread(int, int, SCCXMLWrapper)
	 */
	public void smfread(int iMidi, SCCXMLWrapper scc) {
		smfread(0, iMidi, scc);
	}
	 ...
	 * @param scc 読み込むSCCXMLドキュメント
	 */
	public void smfread(int iMusic, int iMidi, SCCXMLWrapper scc) {
		try {
