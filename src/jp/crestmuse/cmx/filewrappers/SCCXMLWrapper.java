...
public class SCCXMLWrapper extends CMXFileWrapper implements
    PianoRollCompatible {
  /**
  ...
  private class MyMutableNote extends MutableNote {
    ...
    private final MusicXMLWrapper.Note note;
    private final Map<String, String> attr = new HashMap<String, String>();
    private final int graceNum;
    ...
    private MyMutableNote(int onset, int offset, int notenum, int velocity,
      ...
      this(onset, offset, notenum, velocity, offVelocity, ticksPerBeat, note, 0);
    }
    ...
    public int compareTo(MutableMusicEvent another) {
      if (another instanceof MyMutableNote
          && onset() == ((MyMutableNote) another).onset())
        if (note != null && ((MyMutableNote) another).note != null)
          if (note.grace() && ((MyMutableNote) another).note.grace())
            return graceNum - ((MyMutableNote) another).graceNum;
          else if (note.grace() && !((MyMutableNote) another).note.grace())
            return -1;
          else if (!note.grace() && ((MyMutableNote) another).note.grace())
            return 1;
  ...
  /*
   * public void addNoteElement(int onset, int offset, int notenum, int
   * velocity) { checkElementAddition(partStarted); addChild("note");
   * addText(onset + " " + offset + " " + notenum + " " + velocity); //
   * addChild("onset"); // addText(String.valueOf(onset)); //
   * addSibling("offset"); // addText(String.valueOf(offset)); //
   * addSibling("notenum"); // addText(String.valueOf(notenum)); //
   * addSibling("velocity"); // addText(String.valueOf(velocity)); //
  ...
  public void addControlChange(int onset, int offset, int ctrlnum, int value) {
    checkElementAddition(partStarted);
  ...
  public void addNoteElement(int onset, int offset, int notenum, int velocity,
      int offVelocity, MusicXMLWrapper.Note note, int graceNum) {
    checkElementAddition(partStarted);
    lastnote = new MyMutableNote(onset, offset, notenum, velocity, offVelocity,
        getDivision(), note, graceNum);
    mutableNotes.add(lastnote);
  }

  public void addNoteElement(int onset, int offset, int notenum, int velocity,
      int offVelocity, MusicXMLWrapper.Note note) {
    addNoteElement(onset, offset, notenum, velocity, offVelocity, note, 0);
  }
  ...
  /*
   * public void addNoteElement(int onset, int offset, int notenum, int
   * velocity, int offVelocity, MusicXMLWrapper.Note note) {
   * checkElementAddition(partStarted); addChild("note"); addText(onset + " " +
   * offset + " " + notenum + " " + velocity + " " + offVelocity); if (note !=
   * null) putNoteMap(onset, offset, notenum, velocity, offVelocity,
   * getDivision(), currentPart, note); // notemap.put(new MutableNote(onset,
   * offset, notenum, velocity, // offVelocity), note); returnToParent(); }
   */
  ...
  private void putNoteMap(int onset, int offset, int notenum, int velocity,
      int offVelocity, int ticksPerBeat, int partid, MusicXMLWrapper.Note note) {
  ...
  public class Annotation extends NodeInterface implements
      OnsetOffsetCompatible {
    private int onset;
    private int offset;
  ...
  /*
   * public Annotation[] getChordList() { if(chordproglist == null){ Node
   * chordprognode = selectSingleNode("/scc/chord-prog"); if(chordprognode ==
   * null) return null; else{ NodeList chordlist = selectNodeList(chordprognode,
   * "chord"); int size = chordlist.getLength(); chordproglist = new
   * ChordprogElement[size]; for(int i=0; i<size; i++) chordproglist[i] = new
   * ChordprogElement(chordlist.item(i)); } } return chordproglist; }
   */
  ...
  /*
   * public SCCXMLWrapper getSortedSCCXML(int range) throws SAXException,
   * ParserConfigurationException, TransformerException, IOException {
   * SCCXMLWrapper result =
   * (SCCXMLWrapper)CMXFileWrapper.createDocument(TOP_TAG);
   * result.setDivision(getDivision()); Part[] parts = getPartList(); for (Part
   * part : parts) { result.newPart(part.serial(), part.channel(),
   * part.prognum(), part.volume()); SortedSet<Note> notes =
   * part.getSortedNoteSet(range); for (Note note : notes) if (note instanceof
   * ControlChange) result.addControlChange(note.onset(), note.offset(),
   * note.notenum(), note.velocity()); else result.addNoteElement(note.onset(),
   * note.offset(), note.notenum(), note.velocity(),
   * note.getMusicXMLWrapperNote()); result.endPart(); }
   * result.finalizeDocument(); return result; }
   */
  ...
  public MIDIXMLWrapper toMIDIXML() throws ParserConfigurationException,
      TransformerException, SAXException, IOException {
    MIDIXMLWrapper dest = (MIDIXMLWrapper) CMXFileWrapper.createDocument(MIDIXMLWrapper.TOP_TAG);
    toMIDIXML(dest);
    return dest;
  }
    ...
    SCCHandler h = new SCCHandler() {
      ...
      public final void processNote(Note note, SCCXMLWrapper w) {
        if (note instanceof ControlChange) {
          ...
          newscc.addControlChange(cc.onset(), cc.offset(), cc.ctrlnum(),
              cc.value());
        } else {
          byte velocity = it2.next();
          newscc.addNoteElement(note.onset(), note.offset(), note.notenum(),
              velocity, note.getMusicXMLWrapperNote());
        }
      }
    };
    if (sorted)
    ...
    SCCHandler h = new SCCHandler() {
      ...
      public final void processNote(Note note, SCCXMLWrapper w) {
        if (note instanceof ControlChange) {
          ...
          newscc.addNoteElement(note.onset(), note.offset(), note.notenum(),
              vel, note.getMusicXMLWrapperNote());
        }
      }
    };
    if (sorted)
  ...
  public class Part extends NodeInterface {
    ...
    public Note[] getSortedNoteList(final int range) {
      ...
      Arrays.sort(l, new Comparator<Note>() {
        public int compare(Note n1, Note n2) {
          return Math.abs(n1.onset() - n2.onset()) < range ? (n1.notenum() == n2.notenum() ? n1.velocity()
              - n2.velocity()
              : n1.notenum() - n2.notenum())
              : n1.onset() - n2.onset();
        }
    ...
    /*
     * public SortedSet<Note> getSortedNoteSet() { SortedSet<Note> s = new
     * TreeSet<Note>(new Comparator<Note>() { public int compare(Note n1, Note
     * n2) { return n1.onset() == n2.onset() ? (n1.offset() == n2.offset() ?
     * (n1.notenum() == n2.notenum() ? n1.velocity() - n2.velocity() :
     * n1.notenum() - n2.notenum() ) : n1.offset() - n2.offset() ) : n1.onset()
     * - n2.onset(); } }); Note[] notelist = getNoteList(); for (Note note :
     * notelist) s.add(note); // int size = nl.getLength(); // for (int i = 0; i
     * < size; i++) // s.add(new Note(nl.item(i))); return s; }
     */
    /*
     * public SortedSet<Note> getSortedNoteSet(final int range) {
     * SortedSet<Note> s = new TreeSet<Note>(new Comparator<Note>() { public int
     * compare(Note n1, Note n2) { return Math.abs(n1.onset() - n2.onset()) <
     * range ? (n1.notenum() == n2.notenum() ? n1.velocity() - n2.velocity() :
     * n1.notenum() - n2.notenum() ) : n1.onset() - n2.onset(); } }); Note[]
     * notelist = getNoteList(); for (Note note : notelist) s.add(note); //
      ...
      for (Note note : notelist) {
        if (note instanceof ControlChange) {
          el.addEvent(note.onset(), MIDIConst.CONTROL_CHANGE, ch,
              ((ControlChange) note).ctrlnum(), ((ControlChange) note).value());
        } else if (note instanceof PitchBend) {
          el.addEvent(note.onset(), MIDIConst.PITCH_BEND_CHANGE, ch,
              ((PitchBend) note).value(), 0);
        } else {
          el.addEvent(note.onset(), MIDIConst.NOTE_ON, ch, note.notenum(),
              note.velocity());
          el.addEvent(note.offset(), MIDIConst.NOTE_OFF, ch, note.notenum(),
              note.velocity());
        }
      }
      return el;
  ...
  public class Note extends NodeInterface implements NoteCompatible {
    ...
    public MusicXMLWrapper.Note getMusicXMLWrapperNote() {
      NumberedNote note = new NumberedNote(onset, offset, notenum, velocity,
          offVelocity, getDivision(), part.serial());
    ...
    public String toString() {
      return "Note[" + onset + ", " + offset + ", " + notenum + ", " + velocity
          + ", " + offVelocity + "]";
    }
  ...
  /*
   * static class EasyNote { int onset, offset, notenum, velocity, offVelocity;
   * EasyNote(int onset, int offset, int notenum, int velocity) { this(onset,
   * offset, notenum, velocity, velocity); } EasyNote(int onset, int offset, int
   * notenum, int velocity, int offVelocity) { this.onset = onset; this.offset =
   * offset; this.notenum = notenum; this.velocity = velocity; this.offVelocity
   * = offVelocity; } public boolean equals(Object o) { EasyNote en =
   * (EasyNote)o; return ((onset == en.onset) && (offset == en.offset) &&
   * (notenum == en.notenum) && (velocity == en.velocity) && (offVelocity ==
   * offVelocity)); } public int hashCode() { return onset + offset + notenum +
   * velocity + offVelocity; } }
   */

  /*
   * public static void main(String[] args){ try { DeviationInstanceWrapper dev
   * = (DeviationInstanceWrapper)CMXFileWrapper.readfile("deviation.xml");
   * SCCXMLWrapper scc = dev.toSCCXML(480); //scc.write(System.out);
   * scc.beginHeader(); scc.addHeaderElement(10, "hoge", "foo");
   * scc.endHeader(); scc.write(System.out); } catch (Exception e) { // TODO
