 ...
 *document. 
 *<br>
 *<tt>DeviationInstanceWrapper</tt>クラス(仮)は, 
...
public class DeviationInstanceWrapper extends CMXFileWrapper {
  ...
  private HashMap<String,TimewiseControlView> pctrlviews = 
    new HashMap<String,TimewiseControlView>();
  private HashMap<String,TreeView<ExtraNote>> extraNotes = 
    new HashMap<String,TreeView<ExtraNote>>();

  private Map<MusicXMLWrapper.Note,NoteDeviation> noteDevMap = 
    new HashMap<MusicXMLWrapper.Note,NoteDeviation>();
  private Map<MusicXMLWrapper.Note,ChordDeviation> chordDevMap = 
    new HashMap<MusicXMLWrapper.Note,ChordDeviation>();
  private Map<MusicXMLWrapper.Note,MissNote> missNoteMap = 
    new HashMap<MusicXMLWrapper.Note,MissNote>();
...
/**********************************************************************
  ...
  public MusicXMLWrapper getTargetMusicXML() throws IOException {
    if (targetMusicXML == null) {
      try {
        targetMusicXML = 
          (MusicXMLWrapper)readfile(targetMusicXMLDirName + File.separator
                                    + getTargetMusicXMLFileName());
      } catch (FileNotFoundException e) {
        targetMusicXML = 
          (MusicXMLWrapper)readfile(defaultDirName + File.separator 
                                    + getTargetMusicXMLFileName());
      }
    }
...
/*
  public void beginNonPartwiseControls() {
     checkReadOnly();
     checkElementAddition(!nonPartwiseStarted);
     addChild("non-partwise");
  ...
  public void beginPartwiseControls() {
    checkReadOnly();
    checkElementAddition(!partwiseStarted);
    addChild("partwise");
  }

  public void endPartwiseControls() {
    checkElementAddition(partwiseStarted);
    returnToParent();
  }

  public void beginNotewiseDeviations() {
    checkReadOnly();
    checkElementAddition(!notewiseStarted);
    addChild("notewise");
  }
  ...
  void addNoteDeviation(double attack, double release, 
...
*/
  ...
  public double getInitialSilence() {
      String initSil = 
        getDocument().getDocumentElement().getAttribute("init-silence");
...
//      dev.targetMusicXMLFileName = musicxml.getFileName();
  ...
  public NoteDeviation getNoteDeviation(MusicXMLWrapper.Note note) {
    if (noteDevMap.containsKey(note)) {
      ...
      Node linkednode = linkmanager.getNodeLinkedTo
        (note.node(), "note-deviation");
      if (linkednode != null) {
  ...
  public ChordDeviation getChordDeviation(MusicXMLWrapper.Note note) {
    if (chordDevMap.containsKey(note)) {
      ...
      Node linkednode = linkmanager.getNodeLinkedTo
        (note.topNoteOfChord().node(), "chord-deviation");
      if (linkednode != null) {
...
//  }
  ...
  private void analyzePartwiseControls(String partid) {
    ...
    NodeList nl = selectNodeList("/deviation/partwise/part[@id='" + 
                                 partid + "']/measure");
    int size = nl.getLength();
    for (int i = 0; i < size; i++) {
      Node measurenode = nl.item(i);
  ...
  private void analyzeExtraNotes(String partid) {
    ...
    NodeList nl = selectNodeList("/deviation/extra-notes/part[@id='" +
                                 partid + "']/measure");
    int size = nl.getLength();
    for (int i = 0; i < size; i++) {
      Node node = nl.item(i);
    ...
    if (c != null) {
      if (c.type().equals("tempo")) {
        ...
        if (nextTempoDev == null || c.measure() != nextTempoDev.measure() 
          ...
          dest.addHeaderElement(c.timestamp(ticksPerBeat), 
                                "TEMPO", currentTempo);
        }
      } else if (c.type().equals("tempo-deviation")) {
        dest.addHeaderElement(c.timestamp(ticksPerBeat), 
                              "TEMPO",  currentTempo*c.value());
        Control nextTempoDev = 
          tctrlview.lookAhead("tempo", "tempo-deviation");
        if (nextTempoDev == null || nextTempoDev.measure() > c.measure()
            || nextTempoDev.beat() > Math.floor(c.beat()) + 1) {
          int cumulativeTicks = 
            getTargetMusicXML().getCumulativeTicks(c.measure(), 
                                                   ticksPerBeat);
...
/*
  private void controlToSCCHeader(Control c, SCCXMLWrapper dest, 
    ...
    if (c != null) {
      ...
      int cumulativeTicks = 
       musicxml.getCumulativeTicks(c.measure(), ticksPerBeat);
      int timestamp = 
          initticks + cumulativeTicks + (int)(ticksPerBeat * (c.beat()-1));
      if (c.type().equals("tempo")) {
        currentTempo = c.value();
        Control nextTempoDev = tctrlview.lookAhead("tempo-deviation");
        if (nextTempoDev == null || c.measure() != nextTempoDev.measure() || c.beat() != nextTempoDev.beat())
          dest.addHeaderElement(timestamp, "TEMPO", currentTempo);
      } else if (c.type().equals("tempo-deviation")) {
        dest.addHeaderElement(timestamp, "TEMPO", currentTempo * c.value());
        Control nextTempoDev = tctrlview.lookAhead("tempo", "tempo-deviation");
        if (nextTempoDev == null || nextTempoDev.measure() > c.measure() || nextTempoDev.beat() > Math.floor(c.beat()) + 1) {
          int t2 = 
            initticks + cumulativeTicks + ticksPerBeat * 
            (int)(Math.floor(c.beat()));
          dest.addHeaderElement(t2, "TEMPO", currentTempo);
        }
      }
...
*/
  ...
  private void nonPartwiseControlsToSCCHeader(SCCXMLWrapper dest, 
    ...
    while (tctrlview.hasElementsAtNextTime()) {
      controlToSCCHeader(tctrlview.getFirstElementAtNextTime(), 
                         dest, ticksPerBeat);
      while (tctrlview.hasMoreElementsAtSameTime()) 
        controlToSCCHeader(tctrlview.getNextElementAtSameTime(), 
                           dest, ticksPerBeat);
    }
    dest.endHeader();
  }
  ...
  private void processExtraNoteForSCCNoteList(ExtraNote en, 
    ...
    if (en != null) {
      ...
      notelist.list.add(
        new MyNote(onset, offset, en.notenum(), velocity, offVelocity,
                   ticksPerBeat, null)
      );
    }
  }
  ...
  private void processControlForSCCNoteList(Control c, 
    ...
    if (c != null) {
      if (c.type().equals("pedal")) {
        ...
        notelist.list.add(
          new MutableControlChange(c.timestamp(ticksPerBeat), 64, depth, 
                                   ticksPerBeat));
      }
    }
  }
    ...
    getTargetMusicXML().processNotePartwise(new NoteHandlerPartwise() {
      ...
      public void processMusicData(MusicXMLWrapper.MusicData md, 
        ...
        if (md instanceof MusicXMLWrapper.Note) {
          ...
          if (cd == null) cd = getDefaultNoteDeviation();
          NoteDeviationInterface nd = getNoteDeviation(note);
          if (nd == null) nd = getDefaultNoteDeviation();
          ...
          int endDynamics = (int)(baseDynamics * 
                                  cd.endDynamics() * nd.endDynamics());
...
//            int offset = note.actualDuration(ticksPerBeat) + onset;
            notelist.list.add
              (new MyNote(onset + attack, offset + release, note.notenum(), 
               dynamics, endDynamics, ticksPerBeat, note));
          }
        }
      }
    });
  ...
  private void addNoteListToSCC(SCCXMLWrapper dest, 
    ...
    SortedSet<NoteListForSCC> ss = 
      new TreeSet<NoteListForSCC>(
      ...
      for (MutableMusicEvent e : nl.list) {
        if (e instanceof MyNote) {
          ...
          dest.addNoteElement(note.onset(), note.offset(), note.notenum(), 
                              note.velocity(), note.offVelocity(), note.note);
  ...
  public void toSCCXML(SCCXMLWrapper dest, final int ticksPerBeat)
    ...
    Map<String,NoteListForSCC> partwiseNoteList = 
      new HashMap<String,NoteListForSCC>();
...
/*
  public void toSCCXML(final SCCXMLWrapper dest, final int ticksPerBeat) 
    ...
    ParserConfigurationException, SAXException {
    MusicXMLWrapper musicxml = getTargetMusicXML();
    addLinks("//note-deviation", musicxml);
    double initSil = getInitialSilence();
    initticks = (int)Math.round(initSil * ticksPerBeat * 2);
    ...
    if (initticks > 0)
      dest.addHeaderElement(0, "TEMPO", 120);
    controlToSCCHeader(ctrlview.getRoot(), dest, ticksPerBeat);
    while (ctrlview.hasElementsAtNextTime()) {
      controlToSCCHeader(ctrlview.getFirstElementAtNextTime(), 
                         dest, ticksPerBeat);
      while (ctrlview.hasMoreElementsAtSameTime()) 
        controlToSCCHeader(ctrlview.getNextElementAtSameTime(), 
                           dest, ticksPerBeat);
    }
    dest.endHeader();
...
//        private int prevTick = 0;
        public void beginPart(MusicXMLWrapper.Part part, MusicXMLWrapper w) {
          currentPart++;
//          currentTick = 0;
          dest.newPart(currentPart, currentPart, 0, 100);
        }
        public void endPart(MusicXMLWrapper.Part part, MusicXMLWrapper w) {
          dest.endPart();
        }
        public void beginMeasure(MusicXMLWrapper.Measure measure, 
                                 MusicXMLWrapper w) {
//          measure.calcOnsets(ticksPerBeat);
        }
...
//            MusicXMLWrapper.Attributes attr = 
//	      (MusicXMLWrapper.Attributes)md;
//            if (attr.hasChild("divisions"))
//              divisions = attr.getChildTextInt("divisions");
////	  divisions = ((MusicXMLWrapper.Attributes)md).divisions();
//          } else 
          if (md instanceof MusicXMLWrapper.Note) {
            MusicXMLWrapper.Note note = (MusicXMLWrapper.Note)md;
//            Node linkednode = 
...
//            currentTick += note.actualDuration(ticksPerBeat);
////            currentTick += note.duration() * ticksPerBeat / divisions;
//            int offset = currentTick + release;
            if (!note.rest() && getMissNote(note) == null) {
              int onset = note.measure().cumulativeTicks(ticksPerBeat)
                + note.onset(ticksPerBeat);
              int offset = note.actualDuration(ticksPerBeat) + onset;
              dest.addNoteElement(onset + attack, offset + release, 
                                  note.notenum(), dynamics, note);
//              dest.addNoteElement(onset, offset, notenum, dynamics);
            }
//          } else if (md instanceof MusicXMLWrapper.Backup) {
//              currentTick -= 
//                 ((MusicXMLWrapper.Backup)md).actualDuration(ticksPerBeat);
////            currentTick -= ((MusicXMLWrapper.Backup)md).duration()
...
*/
  ...
  private TimewiseControlView getTimewiseControlView() {
    if (tctrlview == null) analyzeControls();
    return tctrlview;
  }
...
/*
    ...
    public final Control getNextControlAtSameTime() {
      return tree.nextL();
    }
    public final boolean hasControlsAtNextTime() {
      return tree.hasNextR();
    }
    public final Control getFirstControlAtNextTime() {
      return tree.nextR();
    }
*/
  ...
  public class Control extends NodeInterface implements Ordered {
    ...
    public final String toString() {
      return "Control (measure: " + measure + ", beat: " + beat
        + ", type: " + type() + 
  ...
  public class NoteDeviation extends NodeInterface 
